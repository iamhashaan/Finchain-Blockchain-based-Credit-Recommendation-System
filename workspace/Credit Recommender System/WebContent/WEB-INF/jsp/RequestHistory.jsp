...
<body>
	...
	<div class="team" id="team">
		...
		<table id="customers" align="center">
			...
			<c:forEach var="policy" items="${result}">

				<tr >
				
					<td class="sp">${policy.lender_name}</td>
					<td hidden="" class="sp1">${policy.borrower_name}</td>
					<td>${policy.type}</td>
					<td>${policy.due}</td>
					<td>${policy.amount}</td>
					<c:if test="${policy.status=='Request Pending' }">
						<td style="color: red; font-weight: bold;">Request Pending</td>
...
<script type="text/javascript">
		$(document).ready(function(c) {
			$('.remove').on('click', function(c) {
				...
				var $text = $row.find(".sp").text();
				var $text1 = $row.find(".sp1").text();

	...
	<div class="team" id="team">
		...
		<table id="customers" align="center">
			...
			<c:forEach var="policy" items="${list}">
				...
				<tr>
<c:if test="${policy.status=='Request Pending' }">
					<td>${policy.lender_name}</td>
					<td>${policy.types}</td>
					<td>${policy.dues}</td>
					<td>${policy.amount}</td>
					
						<td><a href="Accept/${policy.lender_name}"
							style="color: green; font-weight: bold;">Accept</a> <a
							href="Decline/${policy.lender_name}" style="color: red; font-weight: bold;">Decline</a>
						</td>

						<%-- 						<td><a href="DeleteRequest/${policy.lender_name}/${policy.borrower_name}/${type}"><img src="${pageContext.request.contextPath}/resources/images/delete.png" width="70%" class="remove"/> </a></td> --%>


					</c:if>
					
					<c:if test="${policy.status=='Accepted' }">
					<td>${policy.lender_name}</td>
					<td>${policy.types}</td>
					<td>${policy.dues}</td>
					<td>${policy.amount}</td>
					...
					<c:if test="${policy.status=='Accept' }">
					<td>${policy.lender_name}</td>
					<td>${policy.types}</td>
					<td>${policy.dues}</td>
					<td>${policy.amount}</td>
					...
					<c:if test="${policy.status=='Decline' }">
					<td>${policy.lender_name}</td>
					<td>${policy.types}</td>
					<td>${policy.dues}</td>
					<td>${policy.amount}</td>
